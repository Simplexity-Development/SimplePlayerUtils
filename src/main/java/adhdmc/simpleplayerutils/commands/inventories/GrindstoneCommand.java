...
public class GrindstoneCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        //If the sender does not have permission to either open an GRINDSTONE menu for themselves, or others, error and return
        if (!(sender.hasPermission(SPUPerm.GRINDSTONE.getPerm()) || sender.hasPermission(SPUPerm.GRINDSTONE_OTHER.getPerm()))) {
        ...
        //If the sender doesn't have perms to open an GRINDSTONE menu for themselves, and supplies no other player, error and return
        if (args.length == 0 && !sender.hasPermission(SPUPerm.GRINDSTONE.getPerm())) {
            sender.sendMessage(miniMessage.deserialize(SPUMessage.ERROR_NO_PERMISSION.getMessage(),
                    Placeholder.parsed("plugin_prefix", SPUMessage.PLUGIN_PREFIX.getMessage())));
            return false;
        }
        ...
        if (args.length == 1 && sender.hasPermission(SPUPerm.GRINDSTONE_OTHER.getPerm())) {
            ...
            //if Player exists, open their GRINDSTONE menu and return
    ...
    private void openGRINDSTONE(Player player, CommandSender sender){
        ...
        if (sender != null) {
            sender.sendMessage(miniMessage.deserialize(SPUMessage.GRINDSTONE_COMMAND_OTHER.getMessage(),
                    Placeholder.parsed("plugin_prefix", SPUMessage.PLUGIN_PREFIX.getMessage()),
